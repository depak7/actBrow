...
public interface RunRepository extends JpaRepository<RunEntity, String> {
	 ...
	 *
	 * <p>A null {@code claimedAt} counts as orphaned: rows that were already in flight when the
	 * claimed_at column was introduced have no heartbeat, and a plain {@code claimed_at < :before}
	 * comparison would silently exclude them (SQL null semantics), stranding them forever.
	 */
