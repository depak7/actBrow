...
class RunRepositoryCasTests {
	...
	void inFlightRunWithNoHeartbeatCountsAsOrphaned() {
		// Rows that were already IN_PROGRESS when claimed_at was introduced have a null heartbeat.
		// A plain "claimed_at < :before" predicate drops them via SQL null semantics, which would
		// strand every run that was in flight during the deploy.
		...
		// And it must be reclaimable, not just discoverable.
